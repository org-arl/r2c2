...
const TIMEOUT = 10000;             // ms, timeout to get response from to master container
...
if (typeof window.fjage === 'undefined') {
    ...
    window.fjage.getGateway = function (url) {
        const f = window.fjage.gateways.filter(g => (g.sock.url === url));
...
export class Gateway {
    ...
    _websockReconnect() {
        ...
        if (this._firstReConn) this.connObservers.forEach(co => {
            if (co) co(false);
        });
